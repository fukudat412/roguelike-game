...
現在、大規模なリファクタリングを実施中です（**進捗: 65%完了**）

**完了済み**:
- ✅ Phase 1: 基盤整備（インターフェース、BaseUIPanel）
- ✅ Phase 2: SaveManager統一
- ✅ Phase 3: エンティティマネージャー（EnemyManager, ItemManager, ChestManager）
- ✅ Phase 4: ゲームフロー管理（FloorManager, GameStateSerializer）
- ✅ Phase 5: スキル・プレイヤーアクション（SkillExecutor, PlayerActionHandler）
- ✅ Phase 6.1: AchievementSystem抽出

**残り**:
- ⬜ Phase 6.2-6.3: UpgradeSystem, MetaStatistics抽出
- ⬜ Phase 7: Skill.ts分割（各スキルを個別ファイルに）
- ⬜ Phase 8: UI層統合（BaseUIPanel適用）

詳細は [REFACTORING_PLAN.md](./REFACTORING_PLAN.md) を参照。
...
- **ダッシュ機能**: Shift + 移動キーで素早く移動（分かれ道・部屋入口で自動停止）
- **セーブ/ロード**: 進行状況を保存して後から続きをプレイ可能
...
### 🏆 その他の特徴
- **セーブ/ロード機能**: ゲームの進行状況を保存・再開可能
- **ダッシュ機能**: Shiftキーで素早く移動（分かれ道や部屋で自動停止）
...
1. **メインメニュー**
   - 「続きから」: セーブデータがある場合、前回の続きから再開
   - 「新しいゲーム」: ダンジョンを選択して新しい冒険を開始

2. **ダンジョン選択**
   - 4種類のダンジョンから1つを選択
   - 各ダンジョンには固有の敵、環境効果、難易度がある

3. **探索と戦闘**
   - ダンジョンを探索し、敵と戦い、装備を集める
   - スキルを駆使して難しい戦闘を乗り越える
   - デイリーチャレンジを達成して報酬を獲得
...
- `↑` `↓` `←` `→` - 矢印キーでも移動可能
- `2` `4` `6` `8` - テンキーでも移動可能
- `Shift` + 移動キー - ダッシュ（複数マス移動、分かれ道・部屋入口で停止）
...
5. **ダッシュで効率的に探索**: Shift + 移動キーで長い通路を素早く移動
...
5. **ダンジョンごとの特性を理解**: 環境効果を活かした戦略を立てる
6. **こまめにセーブ**: メインメニューに戻る前にセーブする習慣を
